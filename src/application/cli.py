...
def setup_arguments():
    ...
    book_parser.add_argument('--weekday', type=str, required=True, help='Weekday (e.g., mo, tu, we)')
    book_parser.add_argument('--instructor', type=str, required=True, help='Instructor name')
    book_parser.add_argument('--time', type=str, required=True, help='Time (e.g., 18:00)')
    book_parser.add_argument('--studio', type=str, required=True, choices=['CHARLOTTENBURG', 'MITTE', 'PRENZLAUER_BERG'])
    book_parser.add_argument('--spots', type=int, nargs='+', default=[1, 2, 3], help='Preferred spot numbers')
    ...
    credits_parser.add_argument('--add', action='store_true', help='Add a credit package')
    ...
    credits_parser.add_argument('--type', type=str, choices=['usc', 'purchased', 'gift', 'package'], help='Credit type')
    
    # Management commands
    subparsers.add_parser('history', help='Show booking history')
    subparsers.add_parser('cleanup', help='Clean up old bookings')
    
    parser.add_argument('--config', type=str, default='settings.toml', help='Config file path')
    parser.add_argument('--browser-url', type=str, help='Selenium browser URL (overrides config)')
...
def handle_history_command(config: AppConfig):
    ...
    for booking in bookings:
        ...
        logger.info(f"  {status_emoji} {booking.criteria}{spot_info} - {booking.created_at.strftime('%Y-%m-%d %H:%M')}")
