...
class SchedulerService:
                ...
                try:
                    ...
                    if booking:
                        if booking.status.value == "booked":
                            bookings_made += 1
                            logger.info(
                                f"✅ Booked: {criteria} - Spot {booking.spot_number}"
                            )
                        else:
                            logger.warning(
                                f"❌ Failed: {criteria} - {booking.error_message}"
                            )
    ...
    def run_status_check(self):
        ...
        if recent:
            ...
            for booking in recent:
                ...
                spot_info = (
                    f" (spot {booking.spot_number})" if booking.spot_number else ""
                )
    ...
    def cleanup_old_bookings(self, days_old: int = 30):
        ...
        old_bookings = [
            booking for booking in all_bookings if booking.created_at < cutoff_date
        ]
